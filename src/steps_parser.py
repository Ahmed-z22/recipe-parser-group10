...
class StepsParser:
    ...
    def __init__(
        ...
        self.ingredient_names = [
            ing["ingredient_name"].lower() for ing in parsed_ingredients
        ]
        ...
        self.context = {"oven_temperature": None}
        ...
        for direction in self.directions:
            ...
            for sent in sentences:
                ...
                for i, tok in enumerate(sent_doc):
                    if (
                        tok.text.lower() in ["and", "then"]
                        and i > 0
                        and i < len(sent_doc) - 1
                    ):
                        left_has_verb = any(t.pos_ == "VERB" for t in sent_doc[:i])
                        right_has_verb = any(
                            t.pos_ == "VERB" for t in sent_doc[i + 1 :]
                        )
                ...
                if split_points:
                    ...
                    for split_idx in split_points:
                        part_text = " ".join(
                            [t.text for t in sent_doc[start:split_idx]]
                        ).strip()
                        if part_text:
            ...
            if len(words) == 1:
                ...
                if ingredient_name_lower in step_lower:
                    ...
                    significant_words = [
                        w
                        for w in words
                        if len(w) > 3 and w not in ["and", "or", "the", "for"]
                    ]
